		...
		for($i=0;$i<sizeof($arrNames);$i++)
		{
			...
			if($arrTypes[$i]=="file")
			{
				...
				else
				{
					...
					$iFileId=SQLInsertImage($strName);
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			if(strstr($arrTypes[$i],"textbox"))
			{
				...
				list($strType,$strSize)=explode("_",$arrTypes[$i]);
					echo "<input ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")." type=\"text\" id=\"".$arrNames[$i]."\" name=\"".$arrNames[$i]."\" size=\"".$strSize."\"/>";
			...
			if(strstr($arrTypes[$i],"textarea"))
			{
				...
				<textarea ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")." id=\"".$arrNames[$i]."\" name=\"".$arrNames[$i]."\" cols=\"".$strCols."\" rows=\"".$strRows."\"></textarea>";
			}
			else
			...
			if(strstr($arrTypes[$i],"combobox_special"))
			{
				...
				if($arrNames[$i]=="product_category")
				{
						...
						echo "<select ".(isset($_REQUEST["select-width"])?"style=\"width:".$_REQUEST["select-width"]."px\"":"")." name=\"product_category\">";
	...
	while ($myArray = $database->fetch_array($oDataTable))
	{
		...
		if(isset($arrHighlightIds) && isset($strHighlightIdName) && in_array($myArray[$strHighlightIdName],$arrHighlightIds,false)){
			...
			foreach ($oCol as $columnName) 
			{
				...
				if($columnName == "file_date"||$columnName == "date"||$columnName == "timestamp"||$columnName == "date_start")
				{
					if(isset($myArray[$columnName]) && $myArray[$columnName] != "")
					{
						echo "<td>".date($website->GetParam("DATE_HOUR_FORMAT"),$myArray[$columnName])."</td>";
					}
					else
					{
						echo "<td>&nbsp;</td>";
...
function particularCases($columnName,$myArray)
{
	...
	if($columnName=="EmployerLink")
	{
		return "<td><a target=\"_blank\" href=\"../index.php?mod=company&id=".$myArray['id']."\"><img src=\"../images/job-details.png\"/></a></td>"; 
	} 
	else
	if($columnName=="active"||$columnName=="is_resume")
	{  
		global $M_YES,$M_NO;
		return "<td>".($myArray[$columnName]==1?$M_YES:"NO")."</td>"; 
	} 
	else 
	...
	{   
		...
		{	
			...
			{
				$file_show_link = "../user_files/".$myArray['file_id'].".".$c_file_type[1];
				break;
			}
		}

		$strResult = "<td>";    
		if(strstr($myArray['file_name'],".pdf"))  
		{   
			$strResult .= '<a href="'.$file_show_link.'" >  <img src="../images/pdf.gif" width="22" height="22" alt="" border="0">   </a>   ';  
		}  
		else  
		if(strstr($myArray['file_name'],".doc") || strstr($myArray['file_name'],".rtf"))  
		{   
			$strResult .= '   <a href="'.$file_show_link.'">  <img src="../images/doc.gif" width="22" height="22" alt="" border="0">   </a>   ';  
		}  
		else  
		if(strstr($myArray['file_name'],".txt"))  
		{   
			$strResult .= '   <a href="'.$file_show_link.'">  <img src="../images/text.gif" width="17" height="22" alt="" border="0">   </a>   ';  
		}  
		else  
		{   
			global $M_OPEN;
			$strResult .= '   <a href="../file.php?id='.$file_show_link.'">'.$M_OPEN.'</a>   ';  
		}  
		$strResult .= "</td>";   
	...
	if($columnName=="show_status") 
	{
		global $PENDING_VALIDATION,$M_APPROVED,$M_REJECTED;
		$arrst=array($PENDING_VALIDATION,$M_APPROVED,$M_REJECTED);
		return "<td valign=middle>".$arrst[$myArray['status']]."</td>"; 
	} 
	else
	if($columnName=="AdInfo")
	{
		return "<td><a href=\"index.php?category=".$_REQUEST["category"]."&folder=".$_REQUEST["action"]."&page=edit&id=".$myArray['app_id']."\"><img src=\"../images/job-details.png\"/></a></td>"; 
	} 
	else
	...
	if($columnName=="show_images")
	{
		...
		if($myArray['images']!="")
		{
			...
			foreach($image_ids as $image_id)
			{
				...
				elseif(strpos($image_id, "http"))
				{
					$html_content .= "<a href=\"".$image_id."\" class=\"hover\"><img src=\"".$image_id."\" class=\"admin-preview-thumbnail\"/></a>";
				}
			}
	...
	if($columnName=="image_id"){
		global $mod;
				return "<td><a href='../image.php?id=".$myArray['image_id']."' target=_blank><img src='../image.php?id=".$myArray['image_id']."' border=0 width=100 height=100></a></td>";
	}
	...
	{
		$str_field_name="";
		if(isset($myArray['file_id']))
		{
			$str_field_name="file_id";
		}
		else
		{
			$str_field_name="id";
		}
		return "<td><a href='index.php?category=".$_REQUEST["category"]."&folder=".$_REQUEST["action"]."&page=edit&id=".$myArray[$str_field_name]."'><img src='../images/arrow.png' border=0></a></td>";
	}
	else
	...
	if(true)
	{
		...
		for($i=0;$i<sizeof($arrTexts);$i++)
		{
			...
			if(strstr($arrTypes[$i],"textarea")){
				...
				<textarea name=\"".$arrEditFields[$i]."\" id=\"".$arrEditFields[$i]."\" cols=\"".($strCols=="100%"?"20":$strCols)."\" rows=\"".$strRows."\" ".($strCols=="100%"?"style=\"max-width:850px;width:100% !important\"":"").">".stripslashes($strVal)."</textarea>";
			}
			else
			if(strstr($arrTypes[$i],"images"))
			{
				...
				if($strVal!="")
				{
					...
					foreach($image_ids as $image_id)
					{
						...
						elseif(strpos($image_id, "http"))
						{
							echo "<a href=\"".$image_id."\" class=\"hover\"><img src=\"".$image_id."\" class=\"admin-preview-thumbnail\"/></a>";
						}
			...
			if(strstr($arrTypes[$i],"combobox_special"))
			{
				...
				if($arrEditFields[$i]=="product_category")
				{
					   global $LANGUAGE2;
					...
					while (list($key, $val) = each($arrCategories)) 
					{
						echo "<option value=\"".trim($key)."\" ".(strcmp($strVal,trim($key))==0?"selected":"")." >".trim($val)."</option>";
				...
				if($arrEditFields[$i]=="is_resume")  
				{

					$strVal = $oArray[$arrEditFields[$i]];
					global $M_NO,$M_YES;
					echo '<select name="is_resume" style="width:100px">
						<option value="0" '.($strVal==0?"selected":"").'>'.$M_NO.'</option>
						<option value="1" '.($strVal==1?"selected":"").'>'.$M_YES.'</option>
					</select>';
				}
				else
				if($arrEditFields[$i]=="product_category")
