...
function show_job($job)
{
	...
	// if(date('Ymd')==date('Ymd',strip_tags($today))
		// echo '<li>Ajoutée: Aujourd\'hui '.date($website->GetParam("DATE_HOUR_FORMAT"),strip_tags($job["date"])).'</li>';
	// elseif($job["date"]<=$yesterday && $job["date"]>$dayBeforeYesterday)
		// echo '<li>Ajoutée: Hier '.date('\à h:i',strip_tags($job["date"])).'</li>';
	// else
		echo '<li>'.$M_POSTED_ON.': '.date('d/m/y \à h:i',strip_tags($job["date"])).'</li>';
	// echo '<li><strong>'.$job["applications"].'</strong> '.$M_APPLICATIONS.'</li>';
	
	if($job["salary"]!=""&&$job["salary"]!="0")
	{
		echo '<li>'.$M_SALARY.': '.stripslashes($job["salary"]).'</li>';
	}
	else 
		echo '<li>'.$M_SALARY.': A discuter</li>';
	
	echo '</ul>';
	
	echo '<div class="results-job-description">'.$website->text_words(stripslashes(strip_tags($job["message"])),15).'</div>';
	...
	echo '<span class="r-margin-15" style="text-decoration:none;float: left;font-size: 14px;display: block;margin-top: 10px;">Recruteur : '.$job["company"].'</span>';
	echo '<br/><div class="clearfix"></div>';
	
	echo '<div class="row"><div class="col-sm-6"><a href="'.$strLink.'" class="job-details-link underline-link" style="float:none">'.$M_JOB_DETAILS.'</a></div>';
	...
	echo '<a href="'.$strLink.'" class="job-details-link underline-link" style="float:none">'.$M_JOB_DETAILS.'</a>';
	...
	echo '</div>
		...
		echo '<div  class="save-job-link pull-right">';
			...
			echo $M_POSTED_ON.': '.date('d/m/y \à h:i',strip_tags($job["date"]))';
