...
function show_job($job)
{
	...
	echo '<div class="col-sm-2 pull-right">';
		...
		$dattimestape=strip_tags($job["date"]);
		...
		elseif ($jourDate==date('d')-1 && $moisDate==date('m') && $anneeDate==date('Y')){
			...
			echo '<div class="col-sm-12 text-center">';
			echo $jourDate.' '.$mois_fr[$moisDate].' '.$anneeDate;
			echo '</div>';
			echo '<div class="col-sm-12 text-center">';
			echo $heure.':'.$minute;
			echo '</div>';
		}
		echo '<div class="col-sm-12 text-center" style="margin-top: 20px;">';
		if(isset($_COOKIE["saved_listings"]) && strpos($_COOKIE["saved_listings"], $job["id"].",") !== false)
		{
			//ikan if(isset($_REQUEST["mod"])&&$_REQUEST["mod"]=="saved")
			//{
				echo '<a href="javascript:DeleteSavedListing('.$job["id"].')" id="save_'.$job["id"].'" title="Supprimer de mes favoris"><img height="16" src="images/star-a.png" alt="'.$M_SAVE.'"/></a>';
			//}
		}
		else
		{
			echo '<a href="javascript:SaveListing('.$job["id"].')" id="save_'.$job["id"].'" title="Ajouter de mes favoris"><img height="16" src="images/star-d.png" alt="'.$M_SAVE.'"/></a>';
		}	
		echo '</div>'; // end save bouton
	...
	/*	
		...
		elseif($job["logo"]!=""&&preg_match("#http#is", $job["logo"]))
		{
			echo '<img class="img-responsive logo-results" src="'.$job["logo"].'" alt="'.$job["company"].'"/>';
		}
		else
		{
			echo '<img class="img-responsive logo-results" src="images/icon-no-photo.jpg" alt="no-photo"/>';
		}
		echo '</a>';
	// IMAGE
	*/
	...
	echo '<div class="col-sm-8">'; // colunm 8 start
	...
	echo '<ul class="results-job-details row">';
	...
	if($job["region"]!="")
	{
		...
		if($job_location!="")
		{
			echo '<li class="col-md-6 col-xs-12"><i class="fa fa-map-marker"></i>&nbsp;&nbsp; '.$job_location.'</li>';
		}
	}
	// echo '<li>'.$M_POSTED_ON.': '.date($website->GetParam("DATE_HOUR_FORMAT"),strip_tags($job["date"])).'</li>';
	if($job["salary"]!=""&&$job["salary"]!="0")
	{
		echo '<li class="col-md-12 col-xs-12"><i class="fa fa-money"></i>&nbsp;&nbsp; '.$M_SALARY.' : '.stripslashes($job["salary"]).'</li>';
	}
	else 
		echo '<li class="col-md-12 col-xs-12"><i class="fa fa-money"></i>&nbsp;&nbsp; '.$M_SALARY.' : A discuter</li>';
	
	echo '<li class="col-md-12 col-xs-12"><i class="fa fa-buysellads"></i>&nbsp;&nbsp; Ref AD : '.stripslashes($job["Ad ID"]).'</li>';
	echo '</ul>';
	
	echo '<div class="results-job-description">'.$website->text_words(stripslashes(strip_tags($job["message"])),19).'</div>';
	...
	// echo '<a href="'.$strLink.'" class="job-details-link underline-link r-margin-15">'.$job["company"].'</a>';
	// ikan echo '<span class="r-margin-15" style="text-decoration:none;float: left;font-size: 14px;display: block;margin-top: 10px;">RecruteRecruteur : '.$job["company"].'</span>';
	echo '<div class="clearfix"></div>';
	
	/* ikan echo '<div class="row"><div class="col-xs-6"><a href="'.$strLink.'" class="job-details-link underline-link" style="float:none">'.$M_JOB_DETAILS.'</a></div>';
